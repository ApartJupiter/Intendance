...
$qry = $conn->query("SELECT * FROM tp_data where id = ".$_GET['id'])->fetch_array();

foreach($qry as $k => $v){
	$$k = $v;
}

$tprog = $conn->query("SELECT * FROM it_data where project_id = {$id}")->num_rows;
$cprog = $conn->query("SELECT * FROM it_data where project_id = {$id} and status = 3")->num_rows;
$prog = $tprog > 0 ? ($cprog/$tprog) * 100 : 0;
$prog = $prog > 0 ?  number_format($prog,2) : $prog;
$prod = $conn->query("SELECT * FROM usr_activity where project_id = {$id}")->num_rows;
...
<div class="col-lg-12">
			...
			<div class="callout">
				<div class="col-md-12">
					<div class="row">
						<div class="col-sm-6">
							<dl>
								<dt><b class="border-bottom">Project Name</b></dt>
								<dd><?php echo ucwords($name) ?></dd>
								<dt><b class="border-bottom">Project Rating</b></dt>
								<dd><?php echo $tp_rating; ?></dd>
						...
						<div class="col-md-6">
							<dl>
								<dt><b class="border-bottom">Start Date</b></dt>
								<dd><?php echo date("F d, Y",strtotime($start_date)) ?></dd>
							</dl>
							<dl>
								<dt><b class="border-bottom">End Date</b></dt>
								...
								<dd>
									<?php
									  if($stat[$status] =='Pending'){
									  	...
									  	echo "<span class='badge badge-teal'>{$stat[$status]}</span>";
							...
							<dl>
								<dt><b class="border-bottom card-teal">Project Manager</b></dt>
								<dd>
			...
			<div class="card card-outline card-teal">
				<div class="card-header">
					...
					<div class="card-tools">
						<button class="btn btn-teal bg-gradient-teal btn-sm" type="button" id="new_task"><i class="fa fa-plus"></i> New Task</button>
					</div>
				<?php endif; ?>
				</div>
				<div class="card-body p-0">
					...
					<table class="table table-condensed m-0 table-hover">
						...
						<tbody>
							<?php 
							$i = 1;
							$tasks = $conn->query("SELECT * FROM it_data where project_id = {$id} order by task asc");
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card card-outline card-teal">
				<div class="card-header">
					<b>Members Activity</b>
					<div class="card-tools">
						<button class="btn btn-teal bg-gradient-teal btn-sm" type="button" id="new_productivity"><i class="fa fa-plus"></i> New Activity</button>
					</div>
				</div>
				<div class="card-body">
					<?php 
					$progress = $conn->query("SELECT p.*,concat(u.firstname,' ',u.lastname) as uname,t.task FROM usr_activity p inner join users u on u.id = p.user_id inner join it_data t on t.id = p.task_id where p.project_id = $id order by unix_timestamp(p.date_created) desc ");
		                      ...
		                      <div class="user-block">
		                        ...
		                        <span class="description">
		                        	...
		                        	<span class="fa fa-user-clock"></span>
                      				<span><b><?php echo ($row['date_created']) ?></b></span>
